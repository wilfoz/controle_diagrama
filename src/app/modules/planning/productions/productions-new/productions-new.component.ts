...
export class ProductionsNewComponent extends BaseFormsResourceComponent<Production> implements OnInit {
  ...
  submitForm(listFormValue) {
    if (this.resourceForm.valid) {
      ...
      const tower_id = this.towers.find(tower => tower.name === listFormValue.tower)._id
      const activity_id = this.activities.find(activity => activity.name === listFormValue.activity)._id

      const result = Object.assign(listFormValue, { tower_id }, { activity_id });
      this.executeListAction(result);
    }
  }
